          ...
          {serviceDetail.map((item, index) => (
            <div className='relative group w-4/5 sm:w-[300px] h-full overflow-hidden \' key={index}>
              <Image
                ...
                width={300}
                height={400}
                className='w-full h-full group-hover:blur-sm transform rounded-md object-cover group-hover:scale-150 transition-all duration-700 ease-in-out'
              />
              ...
              <p className='absolute inset-0 z-10 opacity-0 group-hover:opacity-100 translate-x-full group-hover:translate-x-0 flex items-center p-2 transition-all duration-700 ease-in-out'>
                {item.h4}
              </p>
